...
/**
 * Admissions Operator Command Center with two-column layout.
 * Answers "What should I focus on today?" with Today's Game Plan as the dominant execution surface.
 */
export function AdmissionsOperatorCommandCenter({ 
  context,
  recommendedAgents = [],
  basePath,
}: { 
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        console.error('Failed to load Admissions Operator data:', error);
      } finally {
        setIsLoading(false);
      }
    };

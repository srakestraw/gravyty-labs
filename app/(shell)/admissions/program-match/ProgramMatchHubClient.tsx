...
export function ProgramMatchHubClient({
  ...
  const scrollToSection = (sectionId: string) => {
    ...
    if (element) {
      // Account for sticky header (pt-14 = 56px, plus some padding)
          ...
          <section id="voice-tone" className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                value={draftConfig?.voiceToneProfileId || ''}
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    console.error('Failed to update voice tone profile:', error);
                  } finally {
              ...
              {draftConfig?.voiceToneProfileId && (
                <p className="text-xs text-gray-500 mt-1">
                  Selected: {voiceToneProfiles.find(p => p.id === draftConfig.voiceToneProfileId)?.name || 'Unknown'}
                </p>
              )}
            </div>
          </section>
